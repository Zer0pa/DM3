...
- **HYBR-01**: Reconstruct a minimal tagged / hybrid layer only after baseline replay and authority-gate definition are complete.
- **HYBR-02**: Reintroduce GPU, micro-transformer, HRM, or scar-engine components one interface at a time behind explicit acceptance tests.

## Out of Scope

| Topic | Reason |
| ----- | ------ |
| Declaring the newer hybrid DM3 recovered | source is still missing |
| Treating manifesto prose as executable evidence | violates falsification-first restart rule |
| Rebuilding the full hybrid stack before baseline replay | architecture-first reset would drift immediately |
| Using RM10 Pro alone as proof of correctness | device execution must remain source-backed and receipt-backed |
...
| ----------- | ----------------------------- | ------------------------------ | ------------------------ | ------------------------ |
| MAPP-01 | `docs/recovery/TEST_REFERENCE_STATUS.md` | `docs/reference/GEOMETRY_FIRST_MANIFESTO.md` | `docs/recovery/RECOVERY_MATRIX.md` | prose-only test citation |
| BASL-02 | host replay ledger | Genesis testing protocol | recoverable October workspace | build success without receipts |
| DEVC-01 | RM10 Pro replay ledger | Genesis testing protocol | Mac host replay | phone shell access without deterministic replay |
| VALD-03 | falsification report | manifesto + mapped registry | host/device ledgers | visual plausibility or partial demos |
